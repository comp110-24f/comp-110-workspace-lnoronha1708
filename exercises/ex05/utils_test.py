...
def test_1_sub() -> None:
    ...
    assert sub(a_list=input, start_idx=1, end_idx=3) == [20, 30]


def test_2_sub() -> None:
    """Tests how sub function mutates input"""
    input: list[int] = [1, 2, 3, 4]
    assert sub(a_list=input, start_idx=-1, end_idx=5) == [1, 2, 3, 4]


def test_3_sub() -> None:
    """Edge case test for sub, if empty list"""
    input: list[int] = []
    assert sub(a_list=input, start_idx=2, end_idx=4) == []


def test_1_add_at_index() -> None:
    """Tests what add_at_index returns"""
    input: list[int] = [1, 2, 3, 5]
    assert add_at_index(list_1=input, element=4, idx_val=3) == None


def test_2_add_at_index() -> None:
    """Tests how add_at_index mutates lists"""
    input: list[int] = [1, 2, 3, 5]
    add_at_index(list_1=input, element=4, idx_val=3)
...
def test_3_add_at_index() -> None:
    ...
    with pytest.raises(IndexError):
        add_at_index(list_1=input, element=1, idx_val=1)
